"""
OrganoidEnv Colab Multi-seed Training (v2)
Simplified for sequential execution on Colab to avoid Ray Tune overhead.

Configuration:
- 3 seeds (42, 123, 456)
- 200 episodes per seed (covers Stages 1-3)
...
CHECKPOINT_FREQ = 25
GDRIVE_DIR = "/content/drive/MyDrive/OrganoidEnv_Results/multiseed_v2"
...
try:
    ...
    if not os.path.exists("/content/drive/MyDrive"):
        ...
        SAVE_DIR = "/content/colab_results_v2"
except ImportError:
    IN_COLAB = False
    SAVE_DIR = LOCAL_DIR
...
def run_seed(seed):
    ...
    np.random.seed(seed)
    env = OrganoidEnv(use_sdm=True, use_morphology=True, use_dual_trace=True, use_stabilizer=True)
    ...
    # CSV Log file setup
    log_file = os.path.join(SAVE_DIR, f"training_log_seed_{seed}.csv")
    start_episode = 0
    cumulative_goals = 0
    ...
    if os.path.exists(log_file) and os.path.getsize(log_file) > 100:
        ...
        try:
            ...
            print(f"Resuming seed {seed} from episode {start_episode}")
        except Exception as e:
            print(f"Could not read existing log, starting from 0: {e}")
            
        # Load agent weights if available
        weight_file = os.path.join(SAVE_DIR, f"agent_weights_seed_{seed}.pth")
        ...
        with open(log_file, "w") as f:
            f.write("seed,episode,reward,success,stage,loss,duration_sec,cumulative_goals\n")
            
    if start_episode >= EPISODES_PER_SEED:
        print(f"Seed {seed} already completed {EPISODES_PER_SEED} episodes. Skipping.")
        return
        
    start_time = time.time()
    
    for episode in range(start_episode, EPISODES_PER_SEED):
        ...
        # Format log line
        log_line = f"{seed},{episode},{total_reward:.4f},{success},{stage},{loss:.4f},{ep_duration:.2f},{cumulative_goals}\n"
        with open(log_file, "a") as f:
            f.write(log_line)
            
        # Console output
        print(f"Seed {seed} | Ep {episode:3d}/{EPISODES_PER_SEED} | Stage {stage} | Reward: {total_reward:6.1f} | Success: {success} | Loss: {loss:.4f} | Time: {ep_duration:.1f}s")
    ...
    print(f"\nFinished Seed {seed} in {total_time/3600:.2f} hours.")
    
    
if __name__ == "__main__":
    print(f"Colab Multi-seed V2. Saving to {SAVE_DIR}")
    for s in SEEDS:
        if SHUTDOWN_REQUESTED:
            break
        run_seed(s)
