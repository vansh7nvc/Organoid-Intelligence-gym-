...
Plasticity, Curriculum Learning
\end{IEEEkeywords}
...
SNN to elicit desired motor behaviors, while plasticity within the
...
$\delta_{\mathrm{spike}} = 0.01$ (normalized units).  Spikes are
...
and advantage streams, Adam optimizer ($\text{lr}=10^{-4}$), target
...
\toprule
\textbf{Configuration} & \textbf{Mean Reward} & \textbf{Var.\ Ratio vs Full} & \textbf{Succ.\ (\%)} & \textbf{Dynamical Effect} \\
\midrule
Full OrganoidEnv       & $\mathbf{+51.00 \pm 24.89}$ & $\mathbf{1.00\times}$ & $\mathbf{71.3\%}$ & Stable sustained learning; balanced homeostatic firing. \\
No GAR (Homeostasis)   & $+23.14 \pm 93.70$          & $3.76\times$          & $86.0\%$          & Severe instability ($\sigma = \pm 93.7$); seizure/coma drops. \\
No SDM (Sparse Mem.)   & $-158.31 \pm 51.28$         & $2.06\times$          & $3.7\%$           & Complete sensory collapse; spatial aliasing failure. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure*}[t]
  ...
  \caption{Multi-seed training progression across $N=3$ biological seeds (200 episodes).
    \textbf{(A)}~Learning curve showing the mean reward with shaded 95\% bootstrap confidence interval band and individual seed trajectories. The red dashed line marks the curriculum injection of obstacles at Episode 100.
    \textbf{(B)}~Cumulative goal completion across seeds (Seeds 123, 42, and 456 achieving 184, 173, and 43 completed goals).}
  \label{fig:multiseed}
\end{figure*}
...
\begin{figure*}[t]
  ...
  \caption{Ablation benchmark comparison ($N=3$ seeds per configuration, 100 episodes).
    \textbf{(A)}~Learning trajectories comparing Full OrganoidEnv (green), No-GAR (red, exhibiting seizure dropouts after Episode 50), and No-SDM (orange, complete sensory collapse).
    \textbf{(B)}~Summary comparison showing mean episode reward with standard deviation error bars alongside task success rates.}
  \label{fig:ablation}
\end{figure*}
...
\begin{figure}[t]
  ...
  \caption{Learning progression and stability across curriculum stages. Moving average reward 
    curves across training episodes demonstrate steady convergence across Stages~1--2 (100 episodes), 
    Stages~1--3 (200 episodes), and the complete 500-episode curriculum despite non-stationary 
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\linewidth]{fig5_baseline_comparison.png}
  \caption{Baseline comparison across four methods ($N=3$ seeds). The OrganoidEnv
    result (solid teal line with shaded confidence band and inset zoom for Episodes 250--500) demonstrates 
    delayed biophysical convergence reaching 70.8\% success. ANN, D3QN-only, and SimpleSNN 
    baselines (dashed/dotted lines) are reported as mean~$\pm$\,std over three seeds.}
  \label{fig:baseline}
\end{figure}
...
% ---------------------------------------------------------------
\subsection{Curriculum Progression and Stage~4 Performance}
% ---------------------------------------------------------------

Across the full 500-episode four-stage curriculum (Fig.~\ref{fig:learning}), the system steadily adapted across progressive non-stationary shifts: achieving $79.0\%$ in Stage~1 (Basic navigation), $97.0\%$ in Stage~2 (Obstacles), $58.0\%$ in Stage~3 (Multi-goal context), and reaching a final asymptotic success rate of $60.7\%$ in Stage~4 (Full Task with obstacles and context-dependent dual-target switching). This $60.7\%$ Stage~4 completion rate explicitly demonstrates that the stabilized SNN can master simultaneous obstacle avoidance and dynamic context routing under strict metabolic and local STDP constraints, before encountering the representational routing limits discussed in Section~\ref{sec:bottleneck}.
...
\begin{figure}[t]
  ...
  \caption{Neural activity during the final curriculum stage.
    \textbf{(A)}~Spike raster plot showing sparse, distributed firing across
    sampled neurons with highlighted motor clusters (Up: index 400--424, Down: index 425--449) over 5000\,ms.
    \textbf{(B)}~Clustered motor neuron activation patterns for the Up (red) and Down
    (blue) quadrants, demonstrating robust directional selectivity (selectivity index~$= 0.64$, $I = 0.31$\,bits).}
  \label{fig:raster}
\end{figure}
...
optimization loop.  This division of labor mirrors proposals in the
...
\item \textbf{Scale:} Real organoids contain $10^5$--$10^6$ neurons
      in 3D with diffusion-mediated signaling. GeNN-based GPU acceleration, 
...
\item \textbf{Synaptic transmission:} Synapses are modeled as
...
This study involved only \emph{in silico} modeling; ethical approval
was not required.
